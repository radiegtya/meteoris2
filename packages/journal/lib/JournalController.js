JournalController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('journal', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Journal.find(this.getCriteria(), sort);
    ...
    view: function() {
        return this.render('journalView', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Journal.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', __("insert_success", "") + ".");
                Router.go('journalView', {_id: _id});
            });
        }
        return this.render('journalInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Journal.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('journalUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Journal.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
