TasksController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('tasks', this.getCriteria(), sort);
    },
...
{finished: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Tasks.find(this.getCriteria(), sort);

        return this.render('tasksIndex', {
            data: {
...
finished: t.find('#finished').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Tasks.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', __("insert_success", "") + ".");
                Router.go('tasksView', {_id: _id});
            });
        }
        return this.render('tasksInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Tasks.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('tasksUpdate', {
            data: {
                model: model,
            }
        });
