SavingController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('saving', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Saving.find(this.getCriteria(), sort);

        return this.render('savingIndex', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Saving.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', __("insert_success", "") + ".");
                Router.go('savingView', {_id: _id});
            });
        }
        return this.render('savingInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Saving.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('savingUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Saving.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
