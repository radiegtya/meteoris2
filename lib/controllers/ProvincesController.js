ProvincesController = MeteorisController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    subscriptions: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();

        this.subscription = this.subs.subscribe('provinces', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Provinces.find(this.getCriteria(), sort);

        return this.render('provincesIndex', {
            data: {
    ...
    view: function() {
        return this.render('provincesView', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Provinces.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Provinces");
                Router.go('provincesView', {_id: _id});
            });
        }
        return this.render('provincesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Provinces.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Provinces");
            });
            Router.go('provincesView', {_id: _id});
        }
        return this.render('provincesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Provinces.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Provinces");
        });
    },
    _loadModel: function(_id) {
        return Provinces.findOne(_id);
    },
});