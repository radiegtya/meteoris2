CommentsController = MeteorisController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    subscriptions: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();

        this.subscription = this.subs.subscribe('comments', this.getCriteria(), sort);
    },
    ...
    getCriteria: function() {
        ...
        return {
            $or: [
                ...
                {speakerId: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Comments.find(this.getCriteria(), sort);

        return this.render('commentsIndex', {
            data: {
    ...
    view: function() {
        return this.render('commentsView', {
            data: {
    ...
    _getDoc: function(t) {
        return {
            ...
            question: t.find('#question').value,
        };
    },
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Comments.insert(doc, function(err, _id) {
                ...
                MeteorisFlash.set('success', "Success Inserting Comments");
                Router.go('commentsView', {_id: _id});
            });
        }
        return this.render('commentsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Comments.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Comments");
            });
            Router.go('commentsView', {_id: _id});
        }
        return this.render('commentsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Comments.remove(_id, function(err) {
            ...
            MeteorisFlash.set('success', "Success Removing Comments");
        });
    },
