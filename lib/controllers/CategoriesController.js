CategoriesController = MeteorisController.extend({
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Categories.find(this.getCriteria(), sort);

        return this.render('categoriesIndex', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Categories.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Categories");
                Router.go('categoriesView', {_id: _id});
            });
        }
        return this.render('categoriesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Categories.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Categories");
            });
            Router.go('categoriesView', {_id: _id});
        }
        return this.render('categoriesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Categories.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Categories");
        });
    },
