WorkshopsSpeakersController = MeteorisController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    subscriptions: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();

        this.subscription = this.subs.subscribe('workshopsSpeakers', this.getCriteria(), sort);
    },
...
{speakerId: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = WorkshopsSpeakers.find(this.getCriteria(), sort);

        return this.render('workshopsSpeakersIndex', {
            data: {
    ...
    view: function() {
        return this.render('workshopsSpeakersView', {
            data: {
...
speakerId: t.find('#speakerId').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            WorkshopsSpeakers.insert(doc, function(err, _id) {
                ...
                MeteorisFlash.set('success', "Success Inserting WorkshopsSpeakers");
                Router.go('workshopsSpeakersView', {_id: _id});
            });
        }
        return this.render('workshopsSpeakersInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            WorkshopsSpeakers.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating WorkshopsSpeakers");
            });
            Router.go('workshopsSpeakersView', {_id: _id});
        }
        return this.render('workshopsSpeakersUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        WorkshopsSpeakers.remove(_id, function(err) {
            ...
            MeteorisFlash.set('success', "Success Removing WorkshopsSpeakers");
        });
    },
    _loadModel: function(_id) {
        return WorkshopsSpeakers.findOne(_id);
    },
});