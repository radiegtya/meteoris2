EmpController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('emp', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Emp.find(this.getCriteria(), sort);
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Emp.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Emp");
                Router.go('empView', {_id: _id});
            });
        }
        return this.render('empInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Emp.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Emp");
            });
            Router.go('empView', {_id: _id});
        }
        return this.render('empUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Emp.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Emp");
        });
    },
    _loadModel: function(_id) {
        return Emp.findOne(_id);
    },
});