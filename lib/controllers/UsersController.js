UsersController = PublicController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    waitOn: function() {
        var sort = PiyikuGridView.getSorting();
    ...
    insert: function(t) {
        Meteor.users.insert(this._getDoc(t), function(err) {
            if (err) {
                PiyikuFlash.set('danger', err);
                throw new Meteor.Error(err);
            }
            PiyikuFlash.set('success', "Success Inserting Posts");
        });
        this._clearForm();
    },
    /* event updating data */
    update: function(t) {
        Meteor.users.update(this.getId(), {$set: this._getDoc(t)}, function(err) {
            ...
            PiyikuFlash.set('success', "Success Updating Posts");
        });
        this._clearForm();
    },
    ...
    remove: function(_id) {
        Meteor.users.remove(_id, function(err) {
            ...
            PiyikuFlash.set('success', "Success Removing Posts");
        });
    },
    ...
    login: function(t) {
        ...
        Meteor.loginWithPassword(email, password, function(err) {
            if (err) {
                PiyikuFlash.set('danger', err.reason);
            } else {
                PiyikuFlash.set('success', 'login success');
                Router.go('/');                
            }
        });
    },
    register: function(t) {
        ...
        Accounts.createUser(doc, function(err){
            if (err) {
                ...
                PiyikuFlash.set('success', 'register success');
                Router.go('/');                
            }
        });
    }
