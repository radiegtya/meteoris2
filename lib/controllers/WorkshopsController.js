WorkshopsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subs.subscribe('workshopsCategories', this.getCriteriaWorkshopsCategories(), sort);
    ...
    getCriteriaWorkshopsCategories: function() {
        return {
            workshopId: this.getId(),
        };
    },
    ...
    view: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var workshopsCategories = WorkshopsCategories.find(this.getCriteriaWorkshopsCategories(), sort);
        ...
        return this.render('workshopsView', {
            data: {
                ...
                ready: this.subscription.ready,
                isEmptyWorkshopsCategories: workshopsCategories.count() === 0 ? true : false,
                workshopsCategories: workshopsCategories,
                hasMoreWorkshopsCategories: this.limit() == workshopsCategories.fetch().length ? this.route.path({limit: this.limit() + this.increment}) : null,
    ...
    insertWorkshopSpeaker: function(t) {
        ...
        WorkshopsSpeakers.insert(doc, function(err, _id) {
            ...
            MeteorisFlash.set('success', "Berhasil menambah pembicara workshop");
        });
    ...
    removeWorkshopSpeaker: function(_id) {
        WorkshopsSpeakers.remove(_id, function(err) {
            ...
            MeteorisFlash.set('success', "Berhasil menghapus pembicara workshop");
        });
    },
    insertWorkshopCategory: function(t) {
        var doc = {
            workshopId: this.getId(),
            categoryId: t.find('#workshopsCategories_form #categoryId').value
        };

        var model = WorkshopsCategories.findOne(doc);
        if (model) {
            var errMessage = "Kategori tidak dapat dimasukkan 2 kali";
            MeteorisFlash.set('danger', errMessage);
            throw new Meteor.Error(errMessage);
        }
        ...
        WorkshopsCategories.insert(doc, function(err, _id) {
            ...
            MeteorisFlash.set('success', "Berhasil menambahkan kategori workshop");
        });

        $('#workshopsCategories_form')[0].reset();
    },
    removeWorkshopCategory: function(_id) {
        WorkshopsCategories.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Berhasil menghapus kategori workshop");
        });
    },
    /* uploading file using cfs:fileSystem package + cfs:ejson */
